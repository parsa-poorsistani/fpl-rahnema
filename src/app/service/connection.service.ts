...
export class ConnectionService implements IConnectionService {
    ...
    async displayFollowings(managerId: objId): Promise<connectionResponse[]|null> {
        ...
        for(let manager of managers){
            let isFollowing:boolean = await connectionRepo.isFollowing(managerId,manager._id);
    ...
    async displayFollowers(managerId: objId): Promise<connectionResponse[] | null> {
        let result:connectionResponse[] = [];
        const managers:IManager[] = await managerRepo.getManagers();
        if(!managers){
            return null;
        ...
        for(let manager of managers){
            let isFollowing:boolean = await connectionRepo.isFollowing(manager._id,managerId);
            if(managerId===manager._id || !isFollowing) {
                continue;
            }
            let data:connectionResponse = {
    ...
    async search(managerId: objId, name: string): Promise<connectionResponse[]|null> {
        let result:connectionResponse[] = [];
        const managers:IManager[]|null = await managerRepo.getManagersByName(name);
        if(!managers){
            return null;
        }
        for(let manager of managers){
    ...
    async searchInFollowers(managerId: objId, name: string): Promise<connectionResponse[] | null> {
        let result:connectionResponse[] = [];
        const managers:IManager[]|null = await managerRepo.getManagersByName(name);
        if(!managers){
            return null;
    ...
    async searchInFollowings(managerId: objId, name: string): Promise<connectionResponse[]|null> {
        let result:connectionResponse[] = [];
        const managers:IManager[]|null = await managerRepo.getManagersByName(name);
        if(!managers){
            return null;
