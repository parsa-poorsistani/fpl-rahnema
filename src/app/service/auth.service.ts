...
import { objId,managerSignUpType,signInputData,authResponseData } from "../Types/types";
...
export class AuthService implements IauthService {
    ...
    async signUpManager(input: signInputData): Promise<string> {
        ...
        const result:string|undefined = await utils.mailSender(email, "confirmation", confirmationCode.toString());
        if(result==='error') {
            return result;
        }
        return result;
    ...
    async verify(email: string, code: string): Promise<authResponseData | string> {
    ...
    async login(username: string, password: string): Promise<authResponseData | string> {
        const manager:IManager|null = await this.managerRepo.findManager(username);
