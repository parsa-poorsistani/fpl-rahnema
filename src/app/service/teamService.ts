...
class TeamService implements ITeamService{
    ...
    async addPlayerToTeam(managerId: objId,playerId: objId,index: number):Promise<string> {
        const team: Array<IPick> = await this.managerRepo.getTeamByManagerId(managerId);        
        ...
        const budget:number = currentBudget - player.now_cost;        
    ...
    async deletePlayerFromTeam(managerId: objId, playerId: objId,index: number): Promise<string> {
        const manager:IManager = await this.managerRepo.getManagerById(managerId);
        const currentBudget:number = manager.budget;
        const player:IPlayer = await this.playerRepo.getPlayerById(playerId);
