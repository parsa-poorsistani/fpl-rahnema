...
class TeamController implements ITeamController {
  ...
  addPlayerToTeam = async (req: Request, res: Response):Promise<Response> => {
    try {
      ...
      const response:string = await this.teamService.addPlayerToTeam(managerId,req.body.id,req.body.index);
  ...
  deletePlayerFromTeam = async (req: Request, res: Response):Promise<Response> => {
    try {
      const managerId: objId = new mongoose.Types.ObjectId(req._id);
